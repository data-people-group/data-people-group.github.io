...
- _Discord community:[ https://discord.com/invite/fHp7B2X3xx](https://discord.gg/docetl)_
...
3. Accuracy: While LLMs like Gemini-1.5-Pro can support prompts of 2 million tokens, simply feeding the entire dataset into an LLM in a single prompt leads to incomplete results. For example, when given the entire dataset at once, Gemini-1.5-Pro extracts some themes from the two most recent debates, but [only reports on the evolution of two themes](https://www.docetl.com/#demo-gemini-output).
...
To optimize document processing pipelines for accuracy, DocETL employs 14 LLM-specific rewrite rules spanning three categories:
